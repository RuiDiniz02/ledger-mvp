...
- **Three kinds of category**, because a monthly budget has three kinds of money in it. `used()` in `lib/data.ts` is the whole rule:
  ...
  - `fixed` is spending you have to make. It costs what was logged, like any other spending. What the kind changes is the *warning*: it never says "running hot" at 80%, because you had to spend it either way, and it reports against the plan ("85% of planned") rather than praising or scolding. The amount does not have to be identical month to month — fuel belongs here.
  - `saving` is a pot, and the only kind whose money is treated as gone before it is spent, because putting 200 into a pot really does remove it from what you can spend this month. The target is that monthly contribution and it accumulates. Taking money out is an expense logged against the pot, which lowers the balance and costs the month nothing, since it was charged to the months that saved it. `potBalance()` answers "how much is in this pot", which no monthly figure can.
...
- **The figure beside a category always means "what you still have".** What was spent goes in the line beneath it, and a word under the figure names it (`left`, `balance`, `spent`), so the same position never means two different things.
- **Taking money out of a pot is reported apart from the month.** It is real spending and it left the account, so the Overview shows it, but it is not added to Spent and does not move Remaining, because it was charged to the months that saved it. Folding it in would make the three headline figures stop reconciling.
